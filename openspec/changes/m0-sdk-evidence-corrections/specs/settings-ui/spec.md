...
### Requirement: 配置中心十分区
系统 SHALL 以**自建居中小窗**提供配置中心（对齐官方 Settings 的 mask + 800×800 面板；rc.6 无整页/路由级 client 槽位；`root` 为 AppFrame 独占且官方禁止注册）：左侧导航十分区（模型/人设/语音合成/语音输入/形象/记忆/成长/工作陪伴/悬浮与桌面/数据与关于），分区懒加载，路由状态经 hash 可分享可恢复，关闭后返回设置页。覆盖层 SHALL 不注册进 `root` 槽位，SHALL NOT 铺满整个视口当独立页面。
